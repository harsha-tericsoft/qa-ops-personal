...
Write-Host "=== ROAM TEST CASE MIGRATION ===" -ForegroundColor Cyan
Write-Host ""
...
foreach ($candidate in $candidates) {
    ...
    try {
        # Use argument array to avoid shell escaping issues
        $arguments = @('update-block', '--graph', 'Project_Kinergy', '--uid', $uid, '--content', $newText)

        # Execute command with token in environment
...
$resultsJson | Out-File -FilePath 'migration-results.json' -Encoding utf8

Write-Host ""
Write-Host "✅ Results saved to: migration-results.json" -ForegroundColor Green
