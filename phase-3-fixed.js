...
/**
 * PHASE 3: LIMITED EXECUTION (FIXED)
 *
 * Uses proper string escaping to handle Unicode characters
...
// Use spawn instead of execSync for better control over string escaping
function updateBlock(uid, text) {
  return new Promise((resolve, reject) => {
    const child = spawn('roam', ['update-block', '--uid', uid, '--string', text, '--graph', GRAPH_NAME], {
      encoding: 'utf8',
      timeout: 10000,
    })
...
// Spawn for reading blocks too
function readBlock(uid) {
  return new Promise((resolve, reject) => {
    const child = spawn('roam', ['get-block', '--uid', uid, '--graph', GRAPH_NAME], {
      encoding: 'utf8',
      timeout: 10000,
    })
    ...
    child.on('close', (code) => {
      if (code === 0) {
        const data = extractJsonFromOutput(stdout)
        resolve(data)
      } else {
        reject(new Error(`Exit code ${code}: ${stderr}`))
      }
...
async function phase3_execution() {
  log('=' .repeat(70))
  log('PHASE 3: LIMITED EXECUTION (FIXED) - Update 50 blocks with verification')
  log('=' .repeat(70))
  log('')
  ...
  const results = {
    ...
    phase: 'PHASE_3_LIMITED_EXECUTION_FIXED',
    graph: GRAPH_NAME,
    blockUpdates: [],
    summary: {
  ...
  log('STEP 1: Updating blocks with proper string escaping...')
  log('')
  ...
  for (let i = 0; i < updatesToApply.length; i++) {
    ...
    try {
      // Use spawn for proper escaping
      await updateBlock(update.uid, update.after)
  ...
  for (let i = 0; i < updatesToApply.length; i++) {
    ...
    try {
      const blockData = await readBlock(update.uid)
...
async function main() {
  try {
    ...
    // Display summary
    log('\n' + '=' .repeat(70))
    log('PHASE 3 VERIFICATION REPORT (FIXED)')
    log('=' .repeat(70))
    log('')
